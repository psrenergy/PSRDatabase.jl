...
"""
    julia_script_from_database(db::DatabaseSQLite, output_jl_path::String, output_db_path::String; path_schema::String = "", path_migrations::String = "")
...
"""
function julia_script_from_database(
    db::DatabaseSQLite,
    ...
    push!(code_lines, "# Generated by PSRDatabase.julia_script_from_database")
    push!(code_lines, "")
    ...
    for (group_id, attr_ids) in groups_map
        ...
        for attr_id in param_attrs
            if haskey(vector_data, attr_id)
                ...
                if any(!ismissing(v) && !_is_null_in_db(v) for v in values)
...
function _format_vector(values::Vector, type::Type)
    ...
    non_null_values = [v for v in values if !ismissing(v) && !_is_null_in_db(v)]
