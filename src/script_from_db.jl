...
"""
    generate_code_from_database(db::DatabaseSQLite, output_jl_path::String, output_db_path::String; path_schema::String = "", path_migrations::String = "")
...
"""
function generate_code_from_database(
    db::DatabaseSQLite,
    ...
    push!(code_lines, "# Generated by PSRDatabase.generate_code_from_database")
    push!(code_lines, "")
    push!(code_lines, "using PSRDatabase")
    push!(code_lines, "using DataFrames")
    push!(code_lines, "using Dates")
    push!(code_lines, "")
    push!(code_lines, "# Assuming db is already loaded or created")
...
function _format_vector(values::Vector, type::Type)
    formatted_values = [_format_value(v, type) for v in values]
