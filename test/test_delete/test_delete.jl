...
function test_delete_element()
    ...
    @test_throws PSRDatabase.DatabaseException PSRDatabase.delete_element!(
        db,
        "Plant",
        "Plant 14",
    )
...
function test_delete_cascade()
    ...
    @test PSRDatabase.read_scalar_relations(db, "ThermalPlant", "Fuel", "id") ==
          ["Fuel 1", "Fuel 2"]
    @test PSRDatabase._get_scalar_relation_map(db, "ThermalPlant", "Fuel", "id") ==
          [1, 2]

    PSRDatabase.delete_element!(db, "Fuel", "Fuel 1")

    @test PSRDatabase.read_scalar_relations(db, "ThermalPlant", "Fuel", "id") ==
          ["", "Fuel 2"]
    @test PSRDatabase._get_scalar_relation_map(db, "ThermalPlant", "Fuel", "id") ==
          [typemin(Int), 1]
    fuel_labels = PSRDatabase.read_scalar_parameters(db, "Fuel", "label")
    ...
    @test PSRDatabase.read_vector_relations(
        db,
        "MultiFuelThermalPlant",
        "Fuel",
        "id",
    ) == [["Fuel 1", "Fuel 2"], ["Fuel 2", "Fuel 3"]]
    @test PSRDatabase.read_scalar_parameters(db, "Fuel", "label") ==
          ["Fuel 2", "Fuel 3", "Fuel 1"]
    @test PSRDatabase._get_vector_relation_map(
        db,
        "MultiFuelThermalPlant",
        "Fuel",
        "id",
    ...
    PSRDatabase.delete_element!(db, "Fuel", "Fuel 2")
